...
// Address + search takes ~90px, offer content ~140px, add inset on top
...
const BANNERS = [
  {
    ...
    headline: 'ITEMS AT',
    highlight: '50% OFF',
    sub: 'Use code BITE50 • Min order ₹199',
    cta: 'Order now',
  ...
  },
  {
    ...
    headline: 'FREE',
    highlight: 'DELIVERY',
    sub: 'On all orders above ₹299 today',
    cta: 'Grab the deal',
    colors: ['#FC8019', '#e06b10'],
    emoji: '🛵',
  },
  {
    id: '3',
    headline: 'NEW USER?',
    highlight: '₹125 OFF',
    sub: 'Your first order is on us!',
    cta: 'Claim now',
    colors: ['#48C479', '#2fa35e'],
    emoji: '🎉',
  },
  {
    id: '4',
    headline: 'MIDNIGHT',
    highlight: 'CRAVINGS?',
    sub: 'We deliver till 2 AM every night',
    cta: "See what's open",
...
export default function HeroBanner({ addressLabel, onAddressPress, onSearchPress, onBannerPress }) {
  ...
  // Total banner height = base + iOS status bar inset
  const BANNER_HEIGHT = BASE_BANNER_HEIGHT + insets.top;

  // Overlay paddingTop = inset so address sits just below status bar
  const OVERLAY_TOP = insets.top + 10;

  // Offer block sits at a fixed distance from bottom
    ...
    <View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <LinearGradient
            colors={item.colors}
            style={[styles.slide, { height: BANNER_HEIGHT }]}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 1 }}
          >
            {/* Burst decoration */}
            ...
            {/* Emoji — right side, vertically centered in offer zone */}
      ...
      {/* ── Address + Search — absolute overlay ───────────────── */}
      <View style={[styles.overlay, { top: OVERLAY_TOP }]} pointerEvents="box-none">
        ...
        <TouchableOpacity style={styles.searchBar} onPress={onSearchPress} activeOpacity={0.9}>
          ...
          <Ionicons name="mic-outline" size={17} color="#888" />
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  // overlay sits on top of the banner
  overlay: {
    position: 'absolute',
    left: 0,
    right: 0,
  ...
  dotActive: { width: 18, backgroundColor: '#E23744' },
  dotInactive: { width: 5, backgroundColor: '#ccc' },
});