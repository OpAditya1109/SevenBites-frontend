 ...
 * a swipeable 2-page pager with a small dot indicator; when only one is
 * active it renders full-width with no swipe needed.
 *
 * Both cards share the exact same CARD_HEIGHT and are wrapped in a dark
 * tinted glass-style card (rgba background) so text stays readable, and a
 * thin brand-tinted border so "order" vs "cart" is still visually
 * distinguishable.
 *
...
export default function HomeFloatingBar({
  ...
  // Tapping a dot jumps the pager straight to that page.
  const goToPage = (idx) => {
    setActiveIndex(idx);
    ...
    <View style={styles.wrap} pointerEvents="box-none">
      {pages.length > 1 ? (
        <>
          ...
          {/* Slim dot indicator — tap a dot to jump to that page. */}
...
const styles = StyleSheet.create({
  // bottom:64 sits just 8px above the (shorter) tab bar
  wrap: {
    position: 'absolute',
    bottom: 64,
    left: 24,
  ...
  // Outer wrapper carries the shadow, inner view carries the glass effect
  // and rounded corners.
  cardShadowWrap: {
    height: CARD_HEIGHT,
    borderRadius: 16,
  ...
  // Dark glass base on every card, then a faint brand-colour wash on top
  // so order/cart are still distinguishable while both stay translucent.
  orderTint: {
    backgroundColor: Platform.select({ ios: 'rgba(20,10,4,0.85)', android: 'rgba(20,10,4,0.9)' }),
    borderColor: 'rgba(252,128,25,0.35)',
  },

  cartTint: {
    backgroundColor: Platform.select({ ios: 'rgba(20,4,6,0.85)', android: 'rgba(20,4,6,0.9)' }),
